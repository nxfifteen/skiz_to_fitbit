<?php
    ...
    class Stats
    {
        ...
        {
            $returnValue = join(", ", $this->getNewMedoClass()->select($this->dbPrefix . 'activity', 'activity', [
                "ORDER" => [ "used" => "DESC" ], "LIMIT" => 3
            ]));
            //nxr(0, $this->getNewMedoClass()->log());
            return $returnValue;
        }
        ...
        {
            $returnValue = join(", ", $this->getNewMedoClass()->select($this->dbPrefix . 'timezone', 'timezone', [
                "ORDER" => [ "used" => "DESC" ], "LIMIT" => 3
            ]));
            //nxr(0, $this->getNewMedoClass()->log());
            return $returnValue;
